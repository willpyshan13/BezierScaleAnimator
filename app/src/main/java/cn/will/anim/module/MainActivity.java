...
public class MainActivity extends AppCompatActivity {
    ...
    private void addGoodsToCart(ImageView goodsImg) {
        ...
        RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) goodsImg.getLayoutParams();
        ((ViewGroup) getWindow().getDecorView()).addView(goods, params);

        // 得到父布局的起始点坐标（用于辅助计算动画开始/结束时的点的坐标）
        int[] parentLocation = new int[2];
        ((ViewGroup) getWindow().getDecorView()).getLocationInWindow(parentLocation);
        ...
        final float startX = startLoc[0];
        final float startY = startLoc[1];

        // 商品掉落后的终点坐标：购物车起始点-父布局起始点+购物车图片的1/5
        float toX = endLoc[0] - goodsImg.getWidth() / 2 + mShoppingCartCountTv.getHeight();
        float toY = endLoc[1] - goodsImg.getHeight() / 2 + mShoppingCartCountTv.getWidth();
        ...
        ValueAnimator scaleAnimator = ValueAnimator.ofFloat(1, 0.1F);
        scaleAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                float value = (Float) valueAnimator.getAnimatedValue();
                if (value == 1) {
                    goods.setTranslationX(startX);
                    goods.setTranslationY(startY);
                }
                goods.setScaleX(value);
                goods.setScaleY(value);
            }
        });
        ...
        set.playSequentially(scaleAnimator, valueAnimator);
        set.start();
        // 动画结束后的处理
        valueAnimator.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                // 把执行动画的商品图片从父布局中移除
                ((ViewGroup) getWindow().getDecorView()).removeView(goods);
            }
